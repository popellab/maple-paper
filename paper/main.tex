...
The extracted targets drew from a range of experimental sources (Supplementary Section~\ref{sec:source-characteristics}). Human clinical data accounted for \sqPrimaryHumanClinicalPct{} of targets, with the remainder from animal studies (in vivo and in vitro). About \spHumantohumanPct{} of targets used human data directly; the rest drew from mouse (\spMousetohumanPct{}) or rat (\spRattohumanPct{}) experiments, which could require cross-species scaling before use in a human QSP model. Only \indExactPct{} of targets came from PDAC-specific experiments; most (\indProxyPct{}) relied on proxy indications such as other cancer types, chronic pancreatitis, or fibrosis models with similar biology.

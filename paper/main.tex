...
About \spHumantohumanPct{} of targets used human data directly (Table~\ref{tab:species}). The rest drew from mouse (\spMousetohumanPct{}) or rat (\spRattohumanPct{}) experiments, which could require cross-species scaling before use in a human QSP model.
...
The source characteristics reveal a fundamental challenge in QSP model calibration. Among SubmodelTargets, only \indExactPct{} came from PDAC-specific experiments; the majority relied on proxy indications. Similarly, \spMousetohumanPct{} required cross-species translation from mouse data. CalibrationTargets face analogous challenges: clinical endpoints from PDAC immunotherapy trials are scarce, and many baseline tumor microenvironment measurements rely on small cohorts or related indications. These are not failures of the extraction process but rather reflections of available literature: disease-specific experiments measuring the exact quantities needed for model calibration are rare.

...
Quantitative systems pharmacology models translate mechanistic understanding of disease biology into predictions of drug response \citep{gadkarSixStageWorkflowRobust2016}. A typical QSP model comprises dozens to hundreds of ordinary differential equations representing cell populations, signaling molecules, and drug pharmacokinetics. Parameterizing these models requires values drawn from experimental literature---proliferation rates, death rates, secretion constants, binding affinities---often fifty or more parameters per model \citep{ribbaMethodologiesQuantitativeSystems2017a}. Proper calibration with uncertainty quantification is essential not only for model credibility but also for downstream applications such as virtual population generation and virtual clinical trials, where parameter distributions directly determine predicted patient heterogeneity \citep{riegerImprovingGenerationSelection2018a, craigPracticalGuideGeneration2023}.

The challenge is not finding relevant papers; it is extracting data systematically with full provenance. Each experiment reports results in different formats, with different units, time points, and uncertainty quantification. Experimental conditions vary across studies and are inconsistently documented. Converting a reported measurement into a calibration constraint requires judgment about applicability, uncertainty, and potential biases. And crucially, the reasoning behind each extraction---why this value, from this paper, interpreted this way---must be documented for reproducibility and review.

Current practice relies on manual curation into spreadsheets. This approach yields poorly documented assumptions, lost provenance, and copy-paste propagation across models. When a reviewer asks ``where did this number come from?'', the answer is often difficult to reconstruct. When a new team member joins, institutional knowledge must be transferred orally. When a parameter needs updating, the original reasoning may be lost.

A further challenge arises when combining constraints from multiple experiments. Parameters often appear across different assays, and the ideal calibration would use all available data simultaneously. But without a structured representation of calibration targets, joint inference requires ad-hoc scripting for each new combination of experiments. There is no standard path from extracted data to inference code.

The rate of relevant publications outpaces what manual curation can accommodate. A scalable approach requires automation, but automation introduces its own risks.
...
\subsubsection{Parameter Databases}

Several databases aggregate quantitative biological parameters from literature. BioNumbers provides over 13,000 entries covering cell sizes, metabolite concentrations, reaction rates, and growth constants, each linked to its source publication \citep{miloBioNumbersDatabaseKey2010}. BRENDA contains over 5 million data points for approximately 90,000 enzymes, including kinetic parameters manually extracted from 157,000 primary literature references \citep{changBRENDAELIXIRCore2021}. Sabio-RK focuses specifically on biochemical reaction kinetics \citep{wittigSABIORKDatabaseBiochemical2012}. For pharmacokinetic modeling, PK-Sim provides physiological parameters for humans and common laboratory animals as part of the Open Systems Pharmacology suite \citep{lippertOpenSystemsPharmacology2019}.

These databases serve their intended purposes well, but they have limitations for QSP calibration. Most store point estimates without uncertainty quantification. Experimental context---the species, cell type, assay conditions, and methodological details that determine whether a value applies to a particular modeling scenario---is inconsistently documented. The databases are organized around biological entities (enzymes, metabolites, organisms) rather than the inference tasks that QSP modelers face. And critically, tracing a value back through intermediate sources to the original experimental measurement often requires substantial manual effort.

\subsubsection{Biomedical Text Mining}

Automated extraction of quantitative information from scientific literature has been a goal of biomedical text mining for decades. Named entity recognition can identify mentions of genes, proteins, diseases, and drugs; relation extraction can link entities into structured assertions; and numerical information extraction can pull quantities from text \citep{zhaoRecentAdvancesBiomedical2021}. Tools like PubTator and SciSpacy provide pre-trained models for biomedical NLP tasks.

The challenge for QSP parameterization is that the relevant information is often embedded in complex experimental descriptions. A reported doubling time depends on cell type, passage number, serum concentration, oxygen tension, and measurement method---details that are scattered across methods sections, figure captions, and supplementary materials. Traditional NLP pipelines struggle with this kind of distributed, context-dependent extraction. Performance on relation extraction benchmarks does not translate directly to the task of assembling coherent calibration targets from heterogeneous experimental reports \citep{leverTextminingClinicallyRelevant2019}.

\subsubsection{Manual Expert Curation}

Manual curation by domain experts remains the gold standard for quality. An experienced modeler can identify the relevant passages, assess methodological quality, judge applicability to the modeling context, and document assumptions. The result is a well-reasoned parameter estimate with clear provenance.

The limitation is scale. A thorough literature review for a single QSP model can require weeks of effort. Documentation quality varies across individuals and projects. When personnel change, institutional knowledge disappears---the reasoning behind a parameter choice may be lost even if the value persists in the model. And the rate of new publications outpaces what manual curation can accommodate; PubMed adds over one million new records annually.

\subsubsection{Large Language Models}

Large language models offer a potential path forward. LLMs can read scientific text, follow structured templates, and extract information at scale. Recent work has explored LLM-assisted workflows for QSP model development, including semi-automated parameter extraction from literature \citep{sainiQSPCopilotAIAugmentedPlatform2025a}. The potential for accelerating model development is substantial.

But LLMs have well-documented failure modes that are particularly problematic for quantitative scientific work. Hallucination---the generation of plausible but incorrect information---is endemic to current models \citep{farquharDetectingHallucinationsLarge2024}. LLMs may fabricate numeric values, invent citations to papers that do not exist, or generate internally inconsistent outputs. These failures are especially dangerous because they often appear fluent and confident, making them difficult to detect through casual review. Studies of LLM hallucination in medical and scientific contexts have found that even state-of-the-art models achieve modest accuracy on detecting subtle factual errors \citep{panditMedHalluComprehensiveBenchmark2025}. For parameter extraction, where the extracted values directly influence model predictions, naive LLM extraction without validation is not acceptable.

\subsubsection{Toward Validated Automation}

These approaches are not mutually exclusive. Databases provide curated reference points. Traditional NLP offers robust entity recognition. Manual curation contributes domain expertise. LLMs provide throughput. The question is whether these can be combined into a workflow that achieves both scale and reliability---using LLMs for rapid extraction while systematically catching their characteristic errors before the results reach downstream inference.

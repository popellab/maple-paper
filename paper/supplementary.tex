...
The forward model (\texttt{model} field) captures the dynamics relevant to the experiment. For time-course data, this is typically a simplified ODE; for steady-state or derived measurements, an algebraic model may suffice.

Code block~\ref{lst:model} shows a forward model using the exponential growth template, together with the surrounding state variables and independent-variable block.
...
\subsection{Measurement Specification}

Each entry of the \texttt{measurements} list specifies the statistical relationship between model predictions and one observed quantity. The \texttt{observation\_code} derives the observed value and uncertainty from raw inputs. The \texttt{observable} defines the transformation from state variables to predicted measurement. The \texttt{likelihood} specifies the distribution family.

Code block~\ref{lst:error-model} shows a complete measurement entry.

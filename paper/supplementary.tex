...
\title{Supplementary Materials:\\Structured Schemas for Provenance-Rich, LLM-Assisted QSP Model Calibration}
...
%=============================================================================
\section{Complete SubmodelTarget Example}
...
\begin{itemize}
    \item \textbf{target\_id}: Unique identifier for the target
    ...
    \begin{itemize}
        ...
        \item \revisedtext{\texttt{forward\_model}: Forward model specification (built-in type or custom). For ODE models, contains nested \texttt{state\_variables} (state variables with initial conditions) and \texttt{independent\_variable} (e.g. time)}
        \item \revisedtext{\texttt{error\_model}: Observable specifications mapping model predictions to data with likelihoods}
...
\revisedtext{The validation framework includes several dozen Pydantic \texttt{@model\_validator} and \texttt{@field\_validator} methods across the SubmodelTarget and CalibrationTarget schemas. They run after field parsing and raise a \texttt{ValidationError} when constraints are violated. The individual validators are fine-grained, but their behaviour is grouped into the ten categories below.}
...
This validator is particularly effective because LLMs that hallucinate values rarely fabricate consistent snippets, so the mismatch provides a reliable detection signal.
...
\subsection{Code Execution Validation}
...
\subsection{Hardcoded Constant Detection}
...
\subsection{Scale Consistency Validation}
...
\subsection{Control Character Validation}
...
\subsection{CalibrationTarget-Specific Validation}
...
%=============================================================================
\section{Generated Julia Code Structure}
...
%=============================================================================
\section{Pydantic Model Definitions}
\label{sec:pydantic-models}
%=============================================================================

This section provides the key Pydantic model definitions that implement both schemas. The full implementations are available in the repository at \texttt{src/qsp\_llm\_workflows/core/calibration/}.
...
To load and validate a target:

\begin{verbatim}
import yaml
from maple.core.calibration import SubmodelTarget
...
class CalibrationTarget(BaseModel):
    ...
    # 30+ CalibrationTarget validators
    # (categorized in Section S3 alongside SubmodelTarget validators)
\end{verbatim}
...
%=============================================================================
\section{SubmodelTarget Schema Details}
...
\subsection{Parameter and Prior Specification}
...
\subsection{Forward Model Specification}

The \texttt{forward\_model} field captures the dynamics relevant to the experiment. For time-course data, this is typically a simplified ODE; for steady-state or derived measurements, an algebraic model may suffice. State variables and the independent-variable block are nested inside \texttt{forward\_model}.
...
\subsection{Error Model Specification}

The \texttt{error\_model} field is a list of measurement entries, each specifying the statistical relationship between model predictions and one observed quantity. The \texttt{observation\_code} derives the observed value and uncertainty from raw inputs. The \texttt{observable} defines the transformation from state variables to predicted measurement. The \texttt{likelihood} specifies the distribution family.
...
%=============================================================================
\section{Complete CalibrationTarget Example}
...
This target constrains baseline intratumoral CD8+ T cell density using immunohistochemistry data from Jansen et al. (2021), who measured CD8 density across 599 pancreatic cancers using tissue microarrays\revisedtext{, of whom 444 had both CD8 IHC and DOG1 staging information and contribute to the mixture-of-lognormals distribution used here}. The observable computes CD8+ density from full model species (effector and exhausted CD8+ T cells) and reference database constants (cancer cell geometry). The distribution code combines two patient subgroups via a mixture of lognormals.
...
%=============================================================================
\section{SubmodelTarget Source Characteristics}
\label{sec:source-characteristics}
%=============================================================================

This section provides detailed breakdowns of source quality, species translation, and indication match for the \revisedtext{\nYAMLTargets{} curated} SubmodelTarget extractions.

\subsection{Source Quality}

Source quality categorizes the type of experimental evidence. \emph{Human clinical} denotes data from human patient samples or clinical measurements. \emph{Human in vitro} denotes experiments using human-derived cell lines or primary human cells in culture. \emph{Animal in vivo} denotes measurements from animal models (e.g., mouse xenografts). \emph{Animal in vitro} denotes experiments using animal-derived cells in culture (e.g., murine splenocyte assays, rat stellate cell cultures).

\input{generated/source_quality_table}

\subsection{Species Translation}

Species translation describes the mapping from the experimental organism to the model target species (human). Targets labeled human$\rightarrow$human required no cross-species translation. Mixed-species entries (e.g., human and rat$\rightarrow$human) indicate that the extraction combined data from multiple species, such as a human clinical measurement anchored by rat-derived kinetic parameters.

\input{generated/species_table}

\subsection{Indication Match}

Indication match describes how closely the experimental disease context matches the PDAC model target. \emph{Exact}: data from PDAC patients or PDAC-derived models. \emph{Proxy}: data from a different indication used as a mechanistic proxy (e.g., other cancer types, chronic pancreatitis, fibrosis models with similar stromal biology). \emph{Related}: data from the same organ or disease class but not a direct match.

\input{generated/indication_table}

%=============================================================================
\section{CalibrationTarget Detailed Metrics}
...
The \ctTotal{} targets drew from \ctUniqueDOIs{} unique primary sources (by DOI), with publication years spanning \ctYearMin{}--\ctYearMax{}. Two sources contributed the majority of targets: a neoadjuvant immunotherapy trial (Li et al., Cancer Cell, 2022) provided data for all \ctTreatment{} GVAX/nivolumab targets, and a large-scale immunohistochemistry study (Liudahl et al., Cancer Discovery, 2021) provided data for 10 baseline targets covering immune cell densities, fractions, and ratios.

\section{Inference Results}
\label{sec:inference-results}

All \nYAMLTargets{} SubmodelTargets were translated to a joint Julia inference script using Turing.jl \citep{geTuringLanguageFlexible2018}. The translator identified \nUniqueParams{} unique parameters and generated corresponding priors and likelihood terms. The plurality of targets (\mtAlgebraicPct{}) use closed-form algebraic forward models; the remainder use structured model types including steady-state, batch accumulation, and first-order decay models (Table~\ref{tab:model-type-dist}).

\input{generated/model_type_table}

The generated script is self-contained, requiring only DifferentialEquations.jl, Turing.jl, and Distributions.jl \citep{JSSv098i16}. Each target contributes a simulate function, observed data constants, and a likelihood term. In this application, most targets constrain individual parameters through algebraic or structured forward models (steady-state, batch accumulation, first-order decay), with \nSharedParams{} parameters shared across multiple targets. The framework supports arbitrarily complex scenarios: multiple derivations constraining the same parameter, single targets constraining multiple parameters, and forward models involving ODEs that cannot be analytically inverted.

\subsection{Convergence Diagnostics}

\begin{revised}\input{snippets/r1m3_supp_inference_fix}\end{revised}
...
Most posteriors are narrower than their priors, indicating that the data constrained the parameters. The tumor growth rate \texttt{k\_C1\_growth} (\KConeGrowthMed{} day$^{-1}$, 90\% CI: \KConeGrowthCILo{}--\KConeGrowthCIHi{}) implies a doubling time of about 130 days, consistent with clinical PDAC volume-doubling-time measurements \citep{ahnTimeProgressionPancreatic2017, jangMissedPancreaticDuctal2015}. The activated PSC death rate \texttt{k\_apsc\_death} (\KApscDeathMed{} day$^{-1}$) implies a half-life of about \revisedtext{18} days.

The IL-2-dependent division counts for CD4+ T cells (\NIltwoCdfourMed{}) fall within the 7--8 generation range expected from CFSE proliferation tracking, where dye dilution limits counting. \revisedtext{The CD8+ estimate (\NIltwoCdeightMed{}) falls below this range, suggesting either a lower IL-2 division capacity for the CD8+ subset in the source data or unmodelled differences between the CD4 and CD8 experimental conditions. Only one calibration target constrains this parameter, its posterior concentrates near the lower end of the prior (Table~\ref{tab:posteriors}) rather than spreading widely, and it shows the lowest tail-ESS in the corpus (Table~\ref{tab:convergence}), so the estimate should be interpreted with caution.}

%=============================================================================
\section{Model-Aware Prompt Construction Details}
\label{sec:prompt-construction}
%=============================================================================

For each target, the prompt injects mechanistic context extracted from the model structure. For target parameters, the system automatically retrieves: the parameter's name, units, and biological description; the reactions where the parameter appears with their rate laws; the species involved in those reactions with descriptions; other parameters appearing in the same reactions; and the broader reaction network showing other reactions that involve the same species. For target observables, the system retrieves the observable's name, units, and description, along with the species and compartments that define it. For example, when searching for data to calibrate a proliferation rate \texttt{k\_apsc\_prolif}, the prompt includes the reaction \texttt{aPSC -> 2*aPSC} with rate law \texttt{k\_apsc\_prolif * aPSC}, species descriptions explaining that aPSC represents activated pancreatic stellate cells, and related parameters like death rates that appear in the same reaction network.
...
%=============================================================================
\section{Schema Implementation Details}
\label{sec:schema-implementation-detail}
%=============================================================================
...
%=============================================================================
\section{Source Relevance Assessment Details}
\label{sec:source-relevance-detail}
%=============================================================================
...
The \texttt{source\_quality} field categorizes the evidence type: \texttt{primary\_\allowbreak human\_\allowbreak clinical}, \texttt{primary\_\allowbreak human\_\allowbreak in\_\allowbreak vitro}, \texttt{primary\_\allowbreak animal\_\allowbreak in\_\allowbreak vivo}, \texttt{primary\_\allowbreak animal\_\allowbreak in\_\allowbreak vitro}, \texttt{review\_\allowbreak article}, or \texttt{textbook}. By default, non-peer-reviewed sources trigger a validation warning; in practice, preprints (e.g., from bioRxiv) that have been publicly available for several months can provide valuable data, and the validator can be configured to accept them with documented justification.
...
\begin{revised}
\subsection{Categories of modeler-supplied context}
\label{sec:curation-categories}

The field-level curation rates reported in Section~\ref{sec:curation} of the main text aggregate changes of several different kinds. Across the SubmodelTarget and CalibrationTarget revisions, modeler-supplied context fell into the following recurring categories:

...
\title{Supplementary Materials:\\Structured Schemas for LLM-Modeler Collaboration in Quantitative Systems Pharmacology Model Calibration}
...
\revisedtext{The validation framework comprises several dozen Pydantic \texttt{@model\_validator} and \texttt{@field\_validator} methods spread across the SubmodelTarget and CalibrationTarget schemas. They run after field parsing and raise a \texttt{ValidationError} with detailed messages when constraints are violated. The implementation is granular (many small validators), but the checks fall into the ten categories below.}
...
The IL-2-dependent division counts for CD4+ T cells (\NIltwoCdfourMed{}) fall within the 7--8 generation range expected from CFSE proliferation tracking, where dye dilution limits counting. \revisedtext{The CD8+ estimate (\NIltwoCdeightMed{}) falls below this range, suggesting either a genuinely lower IL-2 division capacity for the CD8+ subset in the source data or unmodelled differences between the CD4 and CD8 experimental conditions; the wide posterior for this parameter (see Table~\ref{tab:posteriors}) reflects the limited identifiability from a single calibration target.}

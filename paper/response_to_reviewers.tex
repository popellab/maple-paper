...
``Structured Schemas for LLM-Modeler Collaboration in\\
Quantitative Systems Pharmacology Model Calibration''}
...
\response{Agreed that the original framing overclaimed the LLM's role. We have revised the abstract, the closing paragraph of the Introduction, and the opening of Section~\ref{sec:modeler-contribution} to describe the LLM as competent at the scientific tasks it is given when supplied with sufficient context, while characterising the modeler's primary contribution as supplying or correcting the context the LLM cannot infer from the static prompt. The new Section~\ref{sec:curation-categories} enumerates the categories of context the modeler supplies, with concrete examples from this dataset.}

\medskip

Revised abstract closing (replaces the final two sentences of the abstract):

\begin{revised}
\input{snippets/r1m1_abstract_framing}
\end{revised}
...
\begin{revised}
\input{snippets/r1m1_intro_framing}
\end{revised}
...
\begin{revised}
\input{snippets/r1m1_disc_framing}
\end{revised}
...
\begin{revised}
\input{snippets/r1m2_batch_interactive_confounds}
\end{revised}
...
\response{ESS statement corrected in both main text and Supplementary Section~\ref{sec:inference-results} to report the actual bulk- and tail-ESS ranges and to explain the lower tail-ESS values for the hierarchical clone-count parameters. The doubling-time claim now cites the two clinical sources that supplied the underlying prior \citep{ahnTimeProgressionPancreatic2017, jangMissedPancreaticDuctal2015}.}
...
\begin{revised}
\input{snippets/r1m3_main_inference_fix}
\end{revised}
...
\begin{revised}
\input{snippets/r1m3_supp_inference_fix}
\end{revised}
...
\begin{revised}
\input{snippets/r1m4_abstract_caveat}
\end{revised}
...
\begin{revised}
Batch SubmodelTarget extraction used OpenAI \texttt{gpt-5.1} at high reasoning effort; interactive CalibrationTarget extraction and all curation used Anthropic \texttt{claude-opus-4-6} through Claude Code. All extraction and curation runs were performed in February 2026.
\end{revised}
...
\response{The Table~\ref{tab:ct-summary} discrepancy was a loader bug: \texttt{generate\_ct\_stats.py} recursively included 5 retired targets held under \texttt{baseline\_no\_treatment/excluded/}; the loader now skips \texttt{excluded/} paths and the table sums correctly to \ctTotal{}. The remaining apparent inconsistencies came from three distinct cohorts (batch-instrumented SubmodelTargets, \nTargets{}; final curated SubmodelTargets, \nYAMLTargets{}; final CalibrationTargets, \ctTotal{}) that were not consistently named; the abstract, Methods, Results, supplementary, and data-availability statements now use these denominators consistently. The earlier 87-target figure was the curation-paired throughput (a distinct quantity from the inference cohort) and has been removed from the abstract.}
